...
class PoseError {
  public:
    ...
    bool operator()(const T* const q4x1, const T* const t3x1,
        ...
        SensorCalibration::DualQuaternion<T> dq(q, t);
        ...
        SensorCalibration::DualQuaternion<T> dq1(SensorCalibration::AngleAxisToQuaternion<T>(r1), t1);
        SensorCalibration::DualQuaternion<T> dq2(SensorCalibration::AngleAxisToQuaternion<T>(r2), t2);
        SensorCalibration::DualQuaternion<T> dq1_ = dq * dq2 * dq.inverse();

        SensorCalibration::DualQuaternion<T> diff = (dq1.inverse() * dq1_).log();
        residual[0] = diff.real().squaredNorm() + diff.dual().squaredNorm();

//        std::cout << "residual is: "<< residual[0] << std::endl;
        return true;
    }
...
bool HandEyeCalibration::mVerbose = false;
double HandEyeCalibration::mResidualThreshold = 100000;
...
void HandEyeCalibration::estimateHandEyeScrew(
    ...
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d>>& tvecs2,
    Eigen::Matrix4d& H_12, ceres::Solver::Summary& summary, bool planarMotion, bool useInitialGuess) {
    int motionCount = rvecs1.size();
    Eigen::MatrixXd T(motionCount * 6, 8);
    T.setZero();
    ...
    if(useInitialGuess)
    {
        Eigen::Quaterniond rot(H_12.block<3,3>(0,0));
        dq = SensorCalibration::DualQuaterniond(rot, H_12.block<3,1>(0,3));
    }
    else
    {
        dq = estimateHandEyeScrewInitial(T, planarMotion);
        H_12 = dq.toMatrix();
    }
...
void HandEyeCalibration::estimateHandEyeStandalone(
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d>>& rvecs1,
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d>>& tvecs1,
...
void HandEyeCalibration::handEyeTransformRefinement(
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d>>& rvecs1,
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d>>& tvecs1,
...
// docs in header
SensorCalibration::DualQuaterniond
HandEyeCalibration::estimateHandEyeScrewInitial(Eigen::MatrixXd& T,
    ...
    return SensorCalibration::DualQuaterniond(q, d);
}
...
//        double residual = SensorCalibration::HandEye::HandEyeCalibration::computeResidual(p, rvecs1[i], tvecs1[i], rvecs2[i], tvecs2[i]);
//        if(residual > mResidualThreshold)
//            continue;
...
//    std::cout << "Number of examples: "<< rvecs1.size() << std::endl;
//    std::cout << "Number of residual: "<< count << std::endl;
    ...
    Eigen::Vector3d t;
    t << p[4], p[5], p[6];
    dq = SensorCalibration::DualQuaterniond(q, t);
}
...
double HandEyeCalibration::computeResidual(
                double q[7],
...
{
    double residual;
    Eigen::Quaternion<double> qRot(q[0], q[1], q[2], q[3]);
    Eigen::Matrix<double, 3, 1> t;
    t << q[4], q[5], q[6];

    SensorCalibration::DualQuaternion<double> dq(qRot, t);

    Eigen::Matrix<double, 3, 1> r1 = rvecs1.cast<double>();
    Eigen::Matrix<double, 3, 1> t1 = tvecs1.cast<double>();
    Eigen::Matrix<double, 3, 1> r2 = rvecs2.cast<double>();
    Eigen::Matrix<double, 3, 1> t2 = tvecs2.cast<double>();

    SensorCalibration::DualQuaternion<double> dq1(SensorCalibration::AngleAxisToQuaternion<double>(r1), t1);
    SensorCalibration::DualQuaternion<double> dq2(SensorCalibration::AngleAxisToQuaternion<double>(r2), t2);
    ...
    residual = diff.real().squaredNorm() + diff.dual().squaredNorm();
